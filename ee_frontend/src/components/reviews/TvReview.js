...
export default function TvReview(data){
    ...
    const deleteReview = async(id) =>{
        try {
            const response = await fetch(`${apiUrl}/api/v1/tvReviews/${id}`,
                {
                    method: 'DELETE',
                    headers: {
    ...
    return(
        <>
            ...
            {reviews.length !== 0 ? (
                ...
                <Result
                    icon={<MehOutlined/>}
                    title="There are no reviews for this Tv show yet, be the first..."
                />
            )}
        </>
    )
